...
addons.forEach((addon) => {
  addonsHTML += `
    <div class="addonsColumn">
      ...
      <div class="addonName">${addon.name}</div>
      <div class="addonsDetail"> 
        <img src="${addon.image}" class="imgItems" onclick="openImage('${addon.image}')">
        <div>${addon.detail}</div>
      </div>
      <div class="addonPrice">${addon.price}</div>
      <input class="addonsCheckbox" type="checkbox" ${addon.isChecked ? 'checked' : ''}>
    </div>
  `;
});

// JavaScript function to open image in new window
function openImage(imageUrl) {
  window.open(imageUrl, '_blank', 'height=800,width=auto');
}
